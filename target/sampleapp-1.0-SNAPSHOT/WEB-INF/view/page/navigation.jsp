<%-- 
    Document   : navigation
    Created on : Jun 29, 2021, 9:46:49 PM
    Author     : Son_Bui
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>JSP Page</title>
    </head>
    <body>
        <nav class="" style="width: 25%;height: 100%; background-color: #1C2B36;">
            <ul class="navbar-nav text-align-center">
                <li class="nav-item d-flex align-items-center  pl-5 py-2">
                    <i class="fas fa-users text-white mr-2"></i>
                    <a class="nav-link text-white" href="http://localhost:8080/sampleapp/khachhang/list.html">Khách hàng</a>
                </li>
                <li class="nav-item d-flex align-items-center pl-5 py-2">
                    <i class="fas fa-list text-white mr-2"></i>
                    <a class="nav-link text-white" href="http://localhost:8080/sampleapp/sanpham/list.html">Sản phẩm</a>
                </li>
                <li class="nav-item d-flex align-items-center pl-5 py-2">
                    <i class="fas fa-user text-white mr-2"></i>
                    <a class="nav-link text-white" href="http://localhost:8080/sampleapp/taikhoan/list.html">Tài khoản</a>
                </li>
                <li class="nav-item d-flex align-items-center pl-5 py-2">
                    <i class="fas fa-user text-white mr-2"></i>
                    <a class="nav-link text-white" href="http://localhost:8080/sampleapp/muahang/list.html">Mua hàng</a>
                </li>
                <li class="nav-item d-flex align-items-center pl-5 py-2">
                    <i class="fas fa-user text-white mr-2"></i>
                    <a class="nav-link text-white" href="http://localhost:8080/sampleapp/giohang/list.html">Giỏ hàng</a>
                </li>
                <li class="nav-item d-flex align-items-center pl-5 py-2">
                    <i class="far fa-chart-bar text-white mr-2"></i>
                    <a class="nav-link text-white" href="http://localhost:8080/sampleapp/thongkedonhang.html">Thống kê đơn hàng</a>
                </li>

            </ul>
        </nav>
    </body>
</html>
